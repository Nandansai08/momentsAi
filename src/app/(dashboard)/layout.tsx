    ...
    async function loadProfile() {
      ...
      if (user) {
        const { data } = await supabase
          ...
          .single();
        setProfile(
          data || { full_name: user.email?.split("@")[0], plan_type: "free" },
        );
      }
    }
    loadProfile();
    ...
    <div className="min-h-screen bg-background/40 flex flex-col md:flex-row text-foreground selection:bg-violet-100 selection:text-violet-900">
      ...
      <header className="md:hidden h-18 border-b border-border/60 bg-card/80 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-40">
        <Link href="/dashboard" className="flex items-center gap-2">
          ...
          <span className="font-extrabold text-base tracking-tight text-gradient-primary">
            MomentsAI
          </span>
        </Link>
                ...
                {profile && (
                  <div className="flex items-center gap-3 px-1.5">
                    ...
                    <div>
                      ...
                      <span className="text-[8px] px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 font-black uppercase">
                        Pro Member
                      </span>
                    </div>
                  </div>
